...
pure-pursuit expert가 도형을 도는 (state, action) 궤적을 1.5M 스텝만큼 뽑는다. `--perturb_*` 는 궤적에 무작위 위치 킥을 넣어 "이탈 후 복귀" 샘플을 만든다(offline-RL이 복구를 배우려면 필요). `--direction both`(기본)는 각 도형을 **절반은 반시계(CCW), 절반은 시계(CW)** 방향으로 돈다 — 한 방향만 학습하면 반대 방향 경로에서 약해지므로 방향 대칭 데이터를 만든다. (`ccw`/`cw` 로 한 방향만 강제 가능.)
...
python main.py \
  --csv-file ~/drone_simulation/gym_pybullet_drones/gym_pybullet_drones/examples/data/merged.csv \
...
```bash
KMP_DUPLICATE_LIB_OK=TRUE python progress_metric.py "$RUN" 500   # held-out seed (501, 502 로도 확인)
```
...
```
출력: 도형별 expert vs policy 평균 tracking error(m). `--direction both`(기본)면 각 도형을 `triangle-ccw` / `triangle-cw` 처럼 양방향 각각 평가해 시계/반시계 둘 다 잘 도는지 확인한다.
...
## 5-b. DAgger 반복 (평가에서 특정 도형이 코너에서 갇힐 때) — **실제로 우리가 한 것**

5단계 평가에서 **일부 도형만** progress가 낮거나(net laps ≪ 3) 거리 오차가 크게(수십 cm~m) 나오면, 그건 대개 **각진 도형(square/triangle)의 코너 갇힘**이다. 특히 `--direction both` 로 양방향까지 학습하면 코너마다 양방향 복구를 다 배워야 해서 문제가 2배가 되고, 처음 한 번의 DAgger로는 부족할 수 있다. 해결책은 **실패한 도형만 골라 DAgger를 한 번 더** 돌리는 것(= DAgger iteration 2). 원리는 3단계와 같다: **지금 정책으로 몰다가 코너에서 킥으로 이탈시켜 → 그 복구 상태를 pure-pursuit이 라벨**.

실제 예시 — 우리 1.5M 양방향 학습에서 1차 재학습(4단계) 뒤 평가하니 circle·pentagon은 양방향 완주였지만 **square가 양방향 실패(net laps 0.85~1.02, 거리 1.8~2.5m), triangle-CW 실패(0.85m)** 였다. 그래서 실패한 `square triangle` 만 2차 DAgger를 돌렸다:

```bash
conda activate drones && cd ~/drone_simulation/gym_pybullet_drones/gym_pybullet_drones/examples
RUN=$(ls -td ~/drone_simulation/IQL-PyTorch-main/runs/merged/*/ 2>/dev/null | head -1)   # 4단계에서 나온 현재 정책

# (1) 실패한 도형만 골라 2차 DAgger.  --seed-start 는 1차 DAgger(0~59)와 겹치지 않게 60부터.
KMP_DUPLICATE_LIB_OK=TRUE python collect_dagger.py \
  --run-dir "$RUN" \
  --shapes square triangle \
  --seed-start 60 --n-seeds 60 \
  --slew-max-accel 2.0 \
  --perturb_prob 1.0 --perturb_count 8 --perturb_magnitude 1.5 \
  --direction both \
  --output_folder dagger2

# (2) 초기 + 1차 DAgger + 2차 DAgger 의 per-episode CSV 를 한 폴더에 모아 다시 merge
#     (merged.csv 를 이어붙이지 않는 이유는 3단계 설명과 동일 = episode_id 재부여)
rm -rf data_all2/shape_dataset && mkdir -p data_all2/shape_dataset
cp data/shape_dataset/*.csv  dagger/shape_dataset/*.csv  dagger2/shape_dataset/*.csv  data_all2/shape_dataset/
KMP_DUPLICATE_LIB_OK=TRUE python merge_shape_dataset.py \
  --input_folder data_all2/shape_dataset \
  --output_file  data_all2/merged.csv
```

```bash
# (3) data_all2/merged.csv 로 재학습 (4단계와 완전히 같은 명령, --csv-file 만 교체)
conda activate iql && cd ~/drone_simulation/IQL-PyTorch-main
python main.py \
  --csv-file ~/drone_simulation/gym_pybullet_drones/gym_pybullet_drones/examples/data_all2/merged.csv \
  --log-dir  ~/drone_simulation/IQL-PyTorch-main/runs \
  --n-steps 300000 --hidden-dim 256 --beta 3.0 \
  --include-lookahead --reward-clip-min -1.0 \
  --eval-period 100000
```
그다음 **5단계 평가를 다시** 돌려 확인한다. 우리 결과는 이 2차 반복으로 **4도형 × 2방향 = 8/8 모두 완주**(net laps 2.7~3.35, 거리 **≤12.4cm**; square 1.8~2.5m → 0.12m, triangle-CW 0.85m → 0.10m)가 됐다.

> 요령: 매번 4도형을 다 다시 DAgger 할 필요 없이 **평가에서 실패한 도형만** 골라 `--shapes` 에 넣으면 그 도형의 코너 복구 데이터가 집중적으로 쌓여 효율적이다(이미 완주하는 circle/pentagon 은 건드리지 않는다). 여전히 남으면 `--seed-start` 를 더 올려(120, 180 …) 같은 방식으로 3차·4차 반복하면 된다.
...
## 7. 레포에 포함된 학습된 정책 바로 실행

1~4단계(수집·학습·DAgger)를 다시 돌릴 필요 없이, **이미 학습된 정책 2개**로 바로 평가·시각화할 수 있다:

- **소프트** (soft 데이터 + DAgger 2회, 8/8 완주 + 별모양 일반화): `IQL-PyTorch-main/runs_soft_all2/merged/07-15-26_18.02.37_uhsf/`
- **v2** (원본 데이터 + DAgger 2회, 8/8 완주): `IQL-PyTorch-main/runs/merged/07-15-26_14.49.56_fbpq/`
...
# 실행할 모델 폴더를 RUN 에 지정 (아래는 소프트 정책; v2 는 경로만 바꾸면 됨)
RUN=~/drone_simulation/IQL-PyTorch-main/runs_soft_all2/merged/07-15-26_18.02.37_uhsf

# (1) 완주율 — 핵심 지표(net laps). 양방향 각각
KMP_DUPLICATE_LIB_OK=TRUE python progress_metric.py "$RUN" 500 ccw
KMP_DUPLICATE_LIB_OK=TRUE python progress_metric.py "$RUN" 500 cw

# (2) 거리 오차 — expert vs policy, 양방향
KMP_DUPLICATE_LIB_OK=TRUE python evaluate_trained_policy.py \
  --run-dir "$RUN" --shapes triangle square pentagon circle \
  --seed 500 --slew_max_accel 2.0 --direction both --output_folder /tmp/eval
...
# (4) 학습 안 한 별모양(star) 일반화 — 거리 오차 (progress_metric 은 4도형 고정이라 evaluate 사용)
KMP_DUPLICATE_LIB_OK=TRUE python evaluate_trained_policy.py \
  --run-dir "$RUN" --shapes star triangle circle \
  --seed 500 --slew_max_accel 2.0 --direction both --output_folder /tmp/eval_star
```
...
- 위 5·6단계의 `RUN=$(ls -td .../runs/merged/*/ | head -1)`은 "방금 학습한 **최신**"을 자동으로 잡는 것이고, 여기선 특정 **저장된** 모델을 경로로 직접 지정하는 차이다.
