...
## 30. The "−y bias" is a symptom of divergence, not the algorithm or a missing mirror-augment (2026-08-11)

**Background.** A collaborator's frame-stacked model showed a small but consistent world-frame drift (~0.3–0.5 m toward −y) on every shape/direction, and added a left-right **mirror augmentation** to cancel it (hypothesis: an asymmetry baked into the source data). Our IQL policies use no mirror augmentation, so: do they carry the same bias, and where does it actually come from? Diagnostic metric: the per-rollout mean of **`pos_err_y`** = `(target − pos)_y` in **world** frame (`pos_err_y > 0` ⇒ drone sits on the −y side of the path, i.e. pushed toward −y). Measured over **20 held-out seeds (500–519) × both directions × 4 shapes = 160 rollouts** per model, all at D=1.0.

**The bias is entirely a function of whether the policy COMPLETES, not which algorithm:**

| policy | state | `pos_err_y` (mean ± std, n=160) | frac>0 | `mean_ez` |
|---|---|---|---|---|
| IQL soft v2 **INIT** | diverged (LOST) | **+4.07 ± 5.68** | 0.76 | ~+1 |
| IQL soft v2 **FINAL** (DAgger×2) | **completes** | **−0.002 ± 0.008** | 0.44 | ~0 |
| *(another offline-RL variant, local-only)* INIT | diverged | +2.48 ± 6.09 | 0.34 | −1…−3 |
| *(same variant)* after DAgger (still diverged) | diverged | +4.70 ± 5.39 | 0.93 | **−12…−22** |

A completing policy has **zero** bias on every axis (IQL FINAL: |mean| < 0.01 m, frac>0 = 0.44 ≈ chance). Every diverged model — regardless of algorithm — drifts +`pos_err_y` (toward −y); IQL's *diverged* INIT drifts even harder than the other variant's, so this is **not** algorithm-specific. (The variant's runs are deliberately kept out of this repo; cited only as the algorithm-independence control.)

**It is not the data, and not the shape placement.** The soft v2 training data is symmetric to 4 decimals — per-axis means over all 1.5M rows: `pos_err`, `lookahead`, `action`, `velocity` all ≈ 0 (e.g. `mean(ay)=+0.0004`, `mean(ty-y)=+0.0000`). Shape placement is fully randomized per episode (`shape_dataset.py:387–401`): `start_yaw ∈ [0,360)`, `tilt ∈ [0,tilt_max)`, `center ∈ [−m,m]²`, all uniform. So there is **no world-fixed asymmetry in the data or the track**.

**The one world-fixed asymmetry is the drone heading.** `shape_dataset.py:537` sets `INIT_RPY = [0,0,0]` (start yaw = 0, drone faces world +x) and `:663` holds `target_rpy = INIT_RPY[0]` every step — so the track rotates randomly but the **drone body stays locked to world yaw = 0**. Confirmation that the drift rides this world-fixed heading, not the (random-yaw) track: in the original data the track yaw is uniform-random yet `pos_err_y` is consistently + (frac>0 = 0.76) — a track-relative effect would wash to frac ≈ 0.5.
...
**Practical takeaways.** (1) **Mirror augmentation is unnecessary for a completing policy** — our IQL FINAL is bias-free (< 0.01 m) with no mirroring; the collaborator's sub-meter residual was itself a mild-divergence symptom of a model still not perfectly on-path. (2) **Always judge by the progress/completion (net-laps) metric, not by mean |pos_err| or its per-axis mean** — the axis bias is an artifact of divergence, and it vanishes exactly when the policy actually traces the shape (reinforces the project-wide "eval by progress, never mean |pos_err|" rule). (3) A consistent world-frame per-axis `pos_err` mean is a cheap **divergence detector** — near-zero ⇒ tracing; large-and-signed ⇒ the policy is flying off, not merely imprecise.
...
**(a) Real-data downscaling (soft v2).** Recipe fixed: init 300k + DAgger×2, D=1.0, reward-clip −1.0. Subsets are **seed-fixed random episodes drawn to a row budget**, whole-episode (cut at the next `done` after the budget). All four training shapes are present, but "balanced" is only approximate and gets looser as the subset shrinks (actual episode counts — circle / pentagon / square / triangle): **0.5M = 38 / 32 / 32 / 39** (near-even, ±~10%), **0.1M = 8 / 10 / 5 / 5** (skewed — pentagon 2× square/triangle). star is untrained (absent from data). FINAL eval = 50 seeds 500–549 × both dirs (100/shape) + untrained star.
...
*Caveat on max deviation distance:* among LOST runs a larger max/mean dist is **not** a "worse controller" — once the policy diverges it drifts at a roughly constant OOD-saturated command (§30), so accumulated distance ≈ (steps since departure) × drift speed. A bigger number often just means it **left the path earlier** and had more of the episode to fly away, not that the failure mode is more severe. So max-dist ranks *time-of-departure*, not controller quality; only the traverse/completion verdict separates success from failure — don't read the m-scale dist as a severity axis for diverged runs.

**Finding (real downscaling):** cutting the real data to **1/3 (0.5M) or even 1/15 (0.1M, 28 episodes)** leaves FINAL **completion untouched (500/500 all three)**, **progress essentially unchanged** (per-shape net laps within ~0.05 of full — e.g. pentagon 3.22→3.18→3.13, circle 2.70→2.52→2.50), and **cruise precision within noise** (circle 0.016→0.021→0.018; corners 0.09–0.11 like full; tails clean). The INIT still diverges either way and gets *marginally worse* with less data (1/80 → 0/80, laps 0.4–0.7 → 0.3), so data volume does not fix the pre-DAgger coverage hole — but DAgger fixes completion regardless. This is the "**DAgger, not initial-data volume, decides completion**" lesson made quantitative: **15× less real data → identical completion, progress, and precision.** It also survives *initial shape-skew*: 0.1M gives square and triangle only 5 episodes each (vs pentagon's 10), yet both still finish 100/100 — DAgger's per-shape corner collection compensates the uneven starting distribution, not just the total volume. Pipeline wall-clock ~63 min each. Runs: 0.5M init `…_djdp` / FINAL `08-12-26_01.15.29_qgjf`; 0.1M init `…_omxq` / FINAL `08-12-26_03.18.51_ynfe`.

**(b) Generator-augmented downscaling (soft v2).** Can a generative model *substitute* for the missing real data? Train diffusion / GAN on a shrunk subset, sample a **pure 1.5M-row** pool from that generator (24000 windows × 64, `--gen-offpath-frac -1.0`, no real rows in the mix — `build_mix --real-rows 0`), then run the same init 300k + DAgger×2 pipeline on the pure-generated data. This is a harder ask than §28 (which blended generated data into full real): here the generator is the *only* source of the initial distribution, and it was itself trained on as little as 0.1M real rows. **Canonical subsets here are the exactly-balanced shareable ones** (`balanced_subsets/`, 100k = 7 episodes/shape, 500k = 35/shape, seed 0, round-robin interleaved) so the generator-vs-real comparison has no shape-skew confound. FINAL eval identical protocol (50 seeds 500–549 × both + star). *(A skewed-subset diffusion run — the seed-fixed 0.5M of (a) — also reached FINAL 500/500 (triangle 2.64/square 2.84/pentagon 3.13/circle **0.012**/star 3.23), a robustness check that the generator path tolerates the same skew (a) does.)*

**FINAL — net laps mean (min) / traverse / dist mean / p99 (m), per shape × generator × size:**

| shape | diff 100k | diff 500k | GAN 100k | GAN 500k |
|---|---|---|---|---|
| triangle | 2.67 (2.63) / 100 / 0.101 / 0.440 | 2.66 (2.60) / 100 / 0.100 / 0.438 | 2.62 (2.54) / 100 / 0.087 / 0.427 | 2.67 (2.63) / 100 / 0.115 / 0.472 |
| square | 2.89 (2.78) / 100 / 0.114 / 0.438 | 2.88 (2.78) / 100 / 0.116 / 0.443 | 2.86 (2.75) / 100 / 0.096 / 0.427 | 2.87 (2.74) / 100 / 0.135 / 0.486 |
| pentagon | 3.18 (3.02) / 100 / 0.107 / 0.433 | 3.18 (3.01) / 100 / 0.107 / 0.424 | 3.15 (3.00) / 100 / 0.086 / 0.394 | 3.18 (3.04) / 100 / 0.123 / 0.457 |
| circle | 2.62 (2.48) / 100 / **0.010** / 0.034 | 2.57 (2.48) / 100 / **0.016** / 0.061 | 2.36 (2.30) / 100 / 0.024 / 0.072 | 2.72 (2.67) / 100 / **0.013** / 0.049 |
| star *(untrained)* | 3.25 (2.97) / 100 / 0.141 / 0.433 | 3.23 (2.91) / 100 / 0.147 / 0.439 | 3.24 (2.97) / 100 / 0.122 / 0.408 | 3.29 (2.93) / 100 / 0.152 / 0.438 |
| **TOTAL traverse** | **500/500** | **500/500** | **500/500** | **500/500** |
...
**Do the hard generators memorize? (nearest-neighbor test).** The GAN best was grabbed at step 28000, right before a divergence — is that checkpoint overfit/memorizing? Test without retraining: for 2500 generated windows (pos_err over H=64), find each one's nearest-neighbor L2 distance to the real hard-v2 training windows, and compare to the real-to-real NN distribution. Memorization = generated windows sitting at ~0 distance (copies of training examples).

| NN distance to real training windows | median | p25 | min |
|---|---|---|---|
| real → real (baseline) | 0.096 | 0.000 | 0.000 |
| **GAN → real** | **0.912** (9.5×) | 0.238 | 0.055 |
| diff → real | 0.590 (6.1×) | 0.377 | 0.137 |

**Neither generator memorizes** — generated windows are *farther* from the training set than real windows are from each other (GAN 9.5×, diff 6.1× the real intra-window median), the closest GAN window is 0.055 away, and **0%** are near-duplicates (real's p25≈0 is just the many near-identical on-path cruise windows). If anything the GAN sits on the *high-diversity* side (farther than diffusion), consistent with its wider-than-real 38% off-path — the opposite of overfitting. So the step-28000 checkpoint generates novel trajectories, not copies; the GAN's only downstream slip (star 98/100) is OOD-shape character, not checkpoint memorization.

## 33. Single-shape generalization — does one shape teach the others? (init+DAgger×1, 2026-08-13)

**Why 100k + DAgger×1 (not the full pool).** This whole section deliberately uses a *small* budget — ~100k rows and a single DAgger pass — rather than the full-pool recipe. With the full dataset every variant trains up to near-ceiling and the results become **too uniformly good (상향평준화)**, so the good-vs-bad differences between conditions wash out and stop being visible. Deliberately starving the budget keeps the policies on the steep part of the curve where a weaker setup (e.g. circle-only, or diluted mixed) actually shows its weakness — that separation is the entire point of the comparison.

**Why this section exists.** A collaborator asked the inverse of the usual setup: instead of training on all shapes, train on *one* and see whether the rest come along. §14 hinted at it (a triangle-only policy generalized to square/circle but failed pentagon) but never systematically, and never from a single shape + single direction. Here: three separate IQL policies, each trained on **one shape's CW-only data** (~100k rows, freshly generated via `collect_shape_dataset.py --direction cw`, soft-v2 recipe: att_d 1.0, perturb 0.1/2/0.3), **init 300k + DAgger×1 on that one shape only** (`--direction cw`), then each evaluated on **all shapes** (50 seeds, CW). This is the hardest generalization ask in the project — one training shape, one direction, and the eval includes the never-trained pentagon and the never-trained star.

**FINAL — traverse / net laps mean (min) / dist mean (m), per trained-shape × eval-shape (CW, 50 seeds):**

| trained on ↓ | triangle | square | pentagon *(never trained by any)* | circle | star *(never trained)* | TOTAL |
|---|---|---|---|---|---|---|
| **square** | 50/50 · 2.45 (2.07) · 0.146 | 50/50 · 2.68 (2.46) · 0.153 | 50/50 · 2.94 (2.70) · 0.141 | 50/50 · 2.28 (2.02) · 0.059 | 49/50 · 3.12 (0.67) · 0.180 | **249/250** |
| **triangle** | 50/50 · 2.61 (2.47) · 0.133 | 50/50 · 2.81 (2.61) · 0.153 | 50/50 · 3.07 (2.84) · 0.149 | 50/50 · 2.48 (2.26) · 0.035 | 47/50 · 3.27 (1.00) · 0.172 | **247/250** |
| **circle** | 46/50 · 2.37 (1.89) · 0.212 | 50/50 · 2.72 (2.24) · 0.212 | 50/50 · 2.83 (2.58) · 0.220 | 50/50 · 2.63 (2.48) · 0.051 | **21/50** · 1.75 (0.30) · 0.219 | 217/250 |
| **mixed** *(all 4, same budget: ~25k + ~15 DAgger eps each)* | 44/50 · 2.27 (1.47) · 0.174 | 50/50 · 2.54 (2.11) · 0.186 | 50/50 · 2.72 (2.40) · 0.181 | 49/50 · 2.23 (1.97) · 0.086 | 44/50 · 2.67 (0.50) · 0.198 | 237/250 |

All INIT (pre-DAgger) = **0/50** (diverge, as always). The **mixed** row trains on all four shapes at once but at the *same total budget* as a single-shape run (~100k rows and ~60 DAgger episodes split 4 ways, so ~25k + ~15 eps per shape) — see §33c for why it lands *below* single-square/triangle. 
...
| **mixed** | 49/50 · 0.160 | 50/50 · 0.179 | 50/50 · 0.176 | 49/50 · 0.084 | 48/50 · 0.186 | **246/250** | *237/250* |
...
### 33c. Mixed-shape CW baseline at the *same budget* (2026-08-13)

The **mixed** row is folded into the §33 (CW) and §33b (CCW) tables above so all conditions read side by side. Setup: a policy trained on all four shapes CW at once at the **same total budget** as a single-shape run — ~100k rows round-robined across triangle/square/pentagon/circle (~25k each, vs ~100k of *one* shape) and init 300k + DAgger×1 with ~60 episodes TOTAL (15 seeds × 4 shapes, vs 60 on one shape), same soft-v2 recipe, CW; evaluated on all shapes CW and CCW. This isolates the single variable *shape-diversity of the training set* at fixed budget — does spreading the budget over 4 shapes beat concentrating it on 1?

**Finding (§33c) — at a fixed budget, one well-chosen (cornered) shape *beats* mixing four.** The mixed policy scores **237/250 (CW)** — *below* single-**square** (249) and single-**triangle** (247), and only a little above single-**circle** (217). Spreading the same ~100k rows + ~60 DAgger episodes across four shapes leaves each shape with only ~25k rows and ~15 corner-recovery episodes, which is **thinner corner-recovery depth** than 100k+60 concentrated on one cornered shape. And the mixed policy's weak spots are exactly the corner-heavy cases — **triangle 44/50 and star 44/50 on CW** — the same places thin DAgger hurts. So *shape-diversity of the training set does not buy completion when it costs per-shape depth*; what matters is having **enough corner-recovery samples**, and a single cornered shape already supplies the shape-invariant lesson (§33) with full depth. (CCW again slightly higher at 246/250, same direction-symmetry story as §33b: the CW triangle/star seeds it missed are just harder than their CCW mirrors, not a real direction effect.) **Bottom line across §33: to teach all shapes on a tight budget, train deep on one cornered shape, not shallow on many.**

## 34. INIT robustness at proper scale — 50-seed re-evaluation of the pre-DAgger policies (2026-08-13)

**Why this section exists.** Every earlier INIT (pre-DAgger) number was measured on only **10 seeds × both dirs × 4 shapes = /80** — fine for a coarse read, but the actual counts are tiny integers (soft v2 5/80, hard v2 10/80, hard GAN 4/80), so "hard v2 beats soft v2" sat inside the seed noise and couldn't be trusted statistically. This re-evaluates the four INIT policies that matter — **soft v2** (real, gentle kicks, §26/§29), **hard v2** (real, heavy kicks at D=1.0, §29), and the two **hard-v2 generators** (diffusion & GAN, §32) — at the **same 50-seed × both-dirs × 5-shapes = /500** scale as the FINAL sweeps, **reusing the existing INIT run dirs (no retraining, eval only)**, all at D=1.0. This turns the INIT gradient from anecdote into statistics. Runs re-evaluated: soft v2 `…_hgbf`, hard v2 `…_hryq`, hard diff `…_zoeq`, hard GAN `…_nnnl`. Raw: `examples/init_stats50/*.txt`.

**Caveat — these INIT numbers are pessimistic on purpose.** The entire recipe here (data collection, kick strength, hyperparameters, D=1.0) is tuned **assuming DAgger will follow** — DAgger is the completion-decider, so everything upstream is set to feed it, *not* to maximize the standalone INIT policy. Evaluating INIT-only therefore reads **worse than each configuration's true ceiling**: a recipe actually tuned *for* pre-DAgger completion (more/gentler on-path data, different kick balance, INIT-optimal hyperparameters) would score higher across the board. Read this table as a **relative ranking of the datasets' recovery content under a fixed DAgger-oriented recipe**, not as the best any of them can do without DAgger.

**INIT (pre-DAgger, D=1.0) — 50 seeds × both dirs × 5 shapes = /500:**

| INIT dataset | traverse /500 | net laps (mean, per-shape range) | divergence dist mean (m) | worst p99 (m) | prior /80 | verdict |
|---|---|---|---|---|---|---|
| **hard v2** (real, heavy kick) | **95/500 (19.0%)** | 0.95 – 1.92 | **0.8 – 1.3 (bounded ~1 m)** | 3–22 | 10/80 | **least-diverged; the gold standard** |
| **hard GAN** (generator) | 64/500 (12.8%) | 1.17 – 1.82 | 0.5 – 1.3 on trained (**star 5.3**) | 26–66 | 4/80 | bounded on trained shapes, star blows up |
| **soft v2** (real, gentle kick) | 48/500 (9.6%) | 0.52 – 1.46 | **3.3 – 15.4 (flies off)** | 42–65 | 5/80 | LOST — unbounded divergence |
| **hard diff** (generator) | 17/500 (3.4%) | 0.64 – 1.02 | 1.8 – 2.7 | 24–37 | 0/80 | fewest completions, but divergence bounded to a few m |

**Finding (§34a) — with real statistics, hard v2 is the decisive INIT winner on *both* axes.** 95/500 completions (19%, ~2× the next best) **and** the tightest divergence (mean 0.8–1.3 m across every shape — it never flies off). This is the proper-scale confirmation of §29's 10/80 hint: **real heavy-kick recovery data gives the most robust pre-DAgger policy there is.** The generators partly reproduce it — hard GAN's 38%-off-path pool buys 64/500 and ~1 m divergence on the trained shapes — but not fully.

**Finding (§34b) — two *different* failure modes, and completion count alone hides them.** Ranking by completions (hard v2 19% > GAN 12.8% > soft v2 9.6% > diff 3.4%) does **not** match ranking by *how badly it fails when it fails*:
- **soft v2 completes a middling 9.6% but diverges catastrophically** — when it misses it flies **9–15 m off (p99 up to 65 m, max 104 m)**, because gentle kicks left it 0.13% off-path so it has *no recovery content at all*. High-variance: some seeds trace, the rest are lost in orbit.
- **hard diff completes the fewest (3.4%) yet stays bounded (~2–2.7 m)** — the diffusion pool's recovery tail keeps it from flying off, but its recovery is not *aligned* enough to actually close laps. Bounded-but-stuck, the opposite of soft v2's occasionally-completes-but-flies-off.

So "which INIT is best" depends on the axis: **hard v2 wins both**, but between the runners-up, soft v2 buys occasional completions at the price of catastrophic misses, while hard diff buys bounded misses at the price of almost never completing.

**Finding (§34c) — the recovery-rich and recovery-poor policies complete *opposite* shapes.** Per-shape completions invert with corner content:
- **soft v2 (no recovery): best shape = circle (44/100), worst = star (1/100)** — a corner-less INIT can slow-track the smooth circle but has nothing for corners.
...
**Finding (§34e) — LayerNorm is a genuine frontier *push*, not a move along it: 512+LN Pareto-dominates every prior INIT point.** It raises completions to **126/500 (25.2%, the best INIT ever — above no-LN width's 105 and baseline's 95)** *while* collapsing the OOD tail from **max 50–109 m to <4 m** (dist mean 0.63 vs the no-LN 512's 1.9–2.9). So it beats hidden-512 on completions *and* has a ~27× tighter worst-case — it dominates hidden-512, baseline, and even sits near the bounded end of the old frontier while carrying nearly double the completions of the previous bounded champion (tau0.70/rc−5's 68 at max <2.8 m). **256+LN** is a strict improvement over baseline too: same completions (95) but the catastrophic 44 m tail is gone (<3.4 m). This confirms §30's causal story — the flyaways were OOD MLP saturation, and normalizing it away recovers the wider net's completion benefit without the divergence cost. **The INIT frontier was not fundamental after all; it was an artifact of an unnormalized network, and LayerNorm moves the whole frontier outward.** Net update to §34d: LayerNorm (esp. at 512 width) is the one lever that genuinely improves INIT on *both* axes at once. Runs: `…_szro`(256+LN), `…_hkfi`(512+LN); raw `examples/sweep_ln_init/`. Code: `--layernorm` flag ([main.py], [src/util.py:mlp]); eval rebuild patched in [evaluate_trained_policy.py]. (Still DAgger-orthogonal — this improves the *starting* point; whether a better INIT shortens the DAgger needed to reach 500/500 is the open follow-up.)

**Pushing the LN win further — 512+LN (plain depth-2, no wd) is the peak.** Tried to climb past 126 by stacking more capacity/regularization on top of 512+LN (all rc−1, tau0.85, +LN):

| config | traverse /500 | max divergence (m) |
|---|---|---|
| **512 + LN** | **126/500** | <4.0 |
| 512 + LN + weight-decay 1e-4 | 92/500 | <2.9 |
| 512 + LN + n-hidden 3 (depth) | 102/500 | <2.0 |
| 1024 + LN | *(abandoned — ~4× slower on CPU, killed)* | — |

Both add-ons **lower completions** (126→92 / 102) while tightening the tail *further* (max <2.9 / <2.0 m) — i.e. extra regularization just slides back down the (now-LN-shifted) frontier toward the bounded end, which rc−5 already covers. So more is not better on top of LN: **512+LN at the standard depth, no weight decay, is the completion-peak of the improved frontier.** The added `--weight-decay` flag ([main.py]) is kept but stays 0 in the recipe. Runs: `…_qjmn`(wd), `…_pqkw`(h3); raw `examples/sweep_ln2_init/`. **Final INIT verdict: `--layernorm --hidden-dim 512` (else recipe unchanged) is the best pre-DAgger policy found — 126/500 completions with bounded (<4 m) divergence, dominating every non-LN configuration.** This proves §34's caveat quantitatively: the DAgger-oriented recipe (tau=0.85, rc=−1, ovs=0, hidden=256, 300k) is simultaneously near-optimal for INIT completion. There is no cheap *free* hyperparameter INIT win — the pre-DAgger coverage hole is a data/DAgger property, not a tuning artifact, consistent with the project's core "DAgger decides completion" thesis. The actionable INIT knobs are reward-clip and width, both as completion↔bounded-divergence dials (rc−5 / steps-600k tighten tails at a completion cost; hidden-512 buys completions at a tail cost). Revert-to-DAgger values (unchanged, stored in every run's config.json): **tau=0.85, reward-clip-min=−1.0, beta=3.0, n-steps=300k, hidden=256, n-hidden=2, oversample=0.0, include-lookahead, att_d_gain_scale=1.0.** Sweep runs: rc-5 `…_wdgl`(0.85)/`…_zsuq`(0.70)/`…_cgne`(0.55@20-seed); tau0.70 rc-1 `…_sydq`; ovs `…_mdmf`(0.25)/`…_jbux`(0.5); capacity `…_fhsx`(600k)/`…_rywc`(hidden512); combo `…_jtfs`(h512/rc5/tau85)/`…_uzpd`(h512/rc5/tau70). Raw: `examples/sweep_tau_init/`, `examples/sweep_ovs_init/`, `examples/sweep_cap_init/`, `examples/sweep_combo_init/`.

### 34f. Policy ensembling ≈ doubles INIT completion — the real breakthrough to 30–50% (2026-08-14)

**The seed-noise clue.** Chasing tau up (0.85→0.90→0.95) on 512+LN gave 126/91/122 at training-seed 0 — but re-running at **seed 1** gave 104/142/139: the ordering *flipped* (0.90 went worst→best). So the tau differences were **training-seed noise (~±25/500 = ±5%), not a real effect** — 512+LN's true INIT is ~104–142 depending on seed, and "126" was partly seed-0 luck. IQL training is seeded (`--seed`, default 0), so each run is a deterministic draw from this variance. Large seed variance is itself the hint: **average several policies and the variance (and the OOD flyaways behind it) should cancel.** This is the collaborator's ensemble/uncertainty idea (§30 says the flyaways are OOD MLP saturation; independent policies saturate in *different* directions, so their mean cancels OOD error while agreeing on in-distribution recovery).

**Action-space ensembling** (`eval_ensemble.py`, no retraining): at each rollout step, query N trained 512+LN policies, **average their raw target_vel**, then apply one slew cap. Using the six 512+LN/rc−1 models already trained (tau {0.85,0.90,0.95} × seed {0,1}):

| policy | traverse /500 | % | dist mean | max div |
|---|---|---|---|---|
| single 512+LN (best individual) | 126–142 | 25–28% | 0.6 | <4 m |
| ensemble ×2 (tau0.95 seed0+seed1) | 201/500 | 40.2% | 0.35–0.55 | <3.1 m |
...
This is the best INIT-only (pre-DAgger) policy in the project, in the canonical table format.

**Completion vs lap-threshold (same all6 rollouts, recounted).** The 2.0-lap "traverse" bar penalizes rollouts that track fine but run slightly slow; relaxing it reveals how well the policy actually *follows* the path:

| threshold | traverse | % | per-shape (tri/sq/pent/circ/star) |
|---|---|---|---|
| ≥ 2.0 lap (default) | 248/500 | 49.6% | 28/59/80/2/79 |
| **≥ 1.5 lap** | **440/500** | **88.0%** | 80/87/93/**89**/91 |
| ≥ 1.0 lap | 487/500 | 97.4% | — |

The circle "ceiling" (2/100 at 2.0) is an artifact of the 2-lap bar, not a tracking failure: circle's mean is **1.67 laps**, so it nearly always traces the path (89/100 at ≥1.5) but rarely closes two full laps in the fixed episode window. Across all shapes, the INIT ensemble **path-follows at 88% (≥1.5 lap) / 97% (≥1 lap)** — the pre-DAgger policy genuinely tracks every shape; only the strict 2-lap completion count sits at ~50%. (Threshold reporting added to `eval_ensemble.py`.)

**Endurance completion — the speed-confound removed (the true completion rate).** The sub-2-lap rollouts were diagnosed as **SLOW, not escaping** (`diag_ensemble.py`): the closest-path-index keeps advancing to the very end (lastQ_adv 0.25–0.58) and |pos_err| stays ≤2.2 m the whole time — the drone never leaves the path, it just runs at ~55% of time-optimal speed and can't close 2 laps in the fixed 3-lap-time window. So the honest completion metric gives adequate time and asks *does it reach the full 3.0-lap target*. Re-ran the standard sweep (50 seeds × both × 5 shapes) at **n_laps=8** time budget with **early-stop once a rollout hits 3.0 laps** (added `stop_fn` to `shape_dataset.run` — a clean loop-break so env cleanup still runs — and `--n-laps/--stop-at` to `eval_ensemble.py`; early-stop makes it ~as fast as the normal eval since completers exit at 3 laps):

| shape | laps mean (min) | ≥2.0 lap | **≥3.0 lap (full target)** | dist mean | max |
|---|---|---|---|---|---|
| triangle | 2.97 (0.89) | 97/100 | 96/100 | 0.451 | 2.18 |
| square | 2.99 (1.33) | 99/100 | 98/100 | 0.443 | 2.32 |
| pentagon | 3.01 (2.51) | 100/100 | 99/100 | 0.404 | 2.25 |
| circle | 3.00 (3.00) | 100/100 | **100/100** | 0.236 | 2.25 |
| star *(untrained)* | 3.03 (3.00) | 100/100 | 100/100 | 0.471 | 2.08 |
| **TOTAL** | | **496/500 (99.2%)** | **493/500 (98.6%)** | | |

**The true completion rate of the best INIT policy is 98.6% (493/500) at the full 3-lap target — the fixed-time 49.6% was almost entirely a speed artifact.** Given time, the circle "ceiling" vanishes completely (2/100 → **100/100**), and even the never-trained star completes 100/100, all with |pos_err| ≤2.3 m sustained over the long run (no late escape). So the INIT ensemble (no DAgger) essentially **achieves the project's core goal — follow an arbitrary shape from path-relative state alone** — and the only remaining gap is *speed* (~55% of time-optimal), not tracking, stability, or generalization. DAgger's role is now reframed for this policy: not "can it complete" (it already does, given time) but "can it complete *fast enough* to close the laps in the nominal window." Runs/raw: `examples/ensemble_eval/all6_endur.txt`; code `eval_ensemble.py --n-laps 8 --stop-at 3.0`, `shape_dataset.run(stop_fn=…)`. Note the exceptionally clean tails (every shape max <2.2 m, p99 <1.65 m — vs a single no-LN INIT's 109 m) and cruise precision (dist 0.24–0.47 m): even non-completing rollouts stay near the path rather than diverging. Statistics on the number: single 512+LN across the 6 members is **120.7 ± 19.9 /500 (24.1% ± 4.0%)**; the all6 ensemble's 248/500 is **~6σ above** that single-model mean (binomial 95% CI [45.2%, 54.0%]) — the ensemble gain is decisively real, not seed luck.
...
## 35. Best INIT model (all6 ensemble) — evaluation (2026-08-14)

Best pre-DAgger policy = **six 512+LN IQL models (seed {0,1} × τ {0.85,0.90,0.95}) action-averaged** (§34e–f). Standard protocol: seeds 500–549 × both dirs × 5 shapes (incl untrained star), att_d=1.0.

**Fixed-time (normal 3-lap-time window) — traverse @ ≥2.0 laps:**
...
**Endurance (time budget n_laps=8, early-stop at 3 laps) — completion @ full 3.0-lap target, speed confound removed:**

| shape | net laps mean (min) | ≥2.0 lap | ≥3.0 lap | dist mean±std (m) | max |
|---|---|---|---|---|---|
| triangle | 2.97 (0.89) | 97/100 | 96/100 | 0.451±0.077 | 2.180 |
| square | 2.99 (1.33) | 99/100 | 98/100 | 0.443±0.068 | 2.319 |
| pentagon | 3.01 (2.51) | 100/100 | 99/100 | 0.404±0.066 | 2.250 |
| circle | 3.00 (3.00) | 100/100 | 100/100 | 0.236±0.069 | 2.245 |
| star *(untrained)* | 3.03 (3.00) | 100/100 | 100/100 | 0.471±0.050 | 2.081 |
| **TOTAL** | | **496/500 (99.2%)** | **493/500 (98.6%)** | | |

Fixed-time 49.6% (@2-lap) vs endurance 98.6% (@3-lap): the difference is **speed only** (~55% of time-optimal), not tracking — every shape completes the full target given time (circle 2/100 → 100/100), tails stay ≤2.3 m throughout, no escape. Raw: `examples/ensemble_eval/all6.txt`, `examples/ensemble_eval/all6_endur.txt`. See §34f for method, §30 for the OOD-saturation mechanism ensembling cancels.
